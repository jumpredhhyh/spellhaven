...
impl LSystem<OakEntryType> for OakLSystem {
    fn get_start_state(position: Vec3, rng: &mut StdRng) -> Vec<LSystemEntry<OakEntryType>> {
        Self::create_straight_piece(
            ...
            OakEntryType::Stem,
            OakEntryType::Branch { angle_x: 0., angle_z: 0. }
        )
    }
    ...
    fn get_block_from_entry(entry: &LSystemEntry<OakEntryType>) -> BlockType {
        match entry.entry_type {
            OakEntryType::Leaf => BlockType::Grass(100),
            _ => BlockType::Path,
        }
    }
    
    fn recurse_entry(entry: &LSystemEntry<OakEntryType>, rng: &mut StdRng, branches: &mut Vec<LSystemEntry<OakEntryType>>) {
        match entry.entry_type {
            OakEntryType::Branch {
                angle_x,
                angle_z,
